...
function parseAnswers(raw, qKey) {
  return normalizeCollection(raw).map((row, index) => ({
    ...
    next: slug(localizedString(row.next), ""),
...
function parseQuestions(raw) {
  return normalizeCollection(raw).map((row, index) => {
    const key = slug(localizedString(row.q_key), "") || itemIdFromLabel(row.q_text, "") || `q${index + 1}`;
    return {
      key,
...
bindSallaRegistration(BeautyCareAssistant);
