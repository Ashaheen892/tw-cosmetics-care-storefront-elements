...
/* ---- inlined: components/beauty-pao-expiry-calculator/styles.ts ---- */
...
/* ---- inlined: components/beauty-pao-expiry-calculator/types.ts ---- */
type InputMode = 'direct' | 'category';
type ExpiryState = 'safe' | 'warn' | 'expired' | 'future';
type DateFormat = 'long' | 'short' | 'iso';

interface PaoCategory {
  id: string;
  name: string;
  icon: string;
  paoMonths: number;
...
/* ---- inlined: components/beauty-pao-expiry-calculator/utils.ts ---- */
...
function computeExpiry(open: Date, months: number, warnDays: number): ExpiryResult {
...
function formatDate(date: Date, fmt: DateFormat, locale: 'ar' | 'en'): string {
  const pad = (n: number) => String(n).padStart(2, '0');
...
function parseCategories(raw: unknown): PaoCategory[] {
...
function parsePaoOptions(raw: unknown): PaoOption[] {
...
function resolveInputMode(config: Record<string, unknown>): InputMode {
  return getRadioValue(config.bpa_input_mode, 'direct') === 'category' ? 'category' : 'direct';
}

function resolveDateFormat(config: Record<string, unknown>): DateFormat {
...
function paoOptionLabel(option: PaoOption, locale: 'ar' | 'en'): string {
...
function buildIcs(title: string, date: Date): string {
  const pad = (n: number) => String(n).padStart(2, '0');
...
/* ---- inlined: components/beauty-pao-expiry-calculator/index.ts ---- */
...
bindSallaRegistration(BeautyPaoExpiryCalculator as unknown as CustomElementConstructor & { registerSallaComponent?: (tagName: string) => void });
